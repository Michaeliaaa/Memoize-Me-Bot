...
def add_more_qns(update, context):
    if (getText(update) == "/cancel"):
        ...
        print("Add more: No")
        update.message.reply_text("Alright, no more flashcard will be added.", reply_markup=markup)
        try: 
            connection = connect_db()
            cursor = connection.cursor()
            # Delete record 
...
def delete_qns(update, context):
    if (getText(update) == "/cancel"):
        ...
        try:
            ...
            update.message.reply_text('{} flashcard(s) has been deleted successfully.'.format(len(list_of_card_id)))
            # Delete index table
            postgresSQL_delete_query = """DROP TABLE index"""
            cursor.execute(postgresSQL_delete_query)
            connection.commit()
