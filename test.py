...
CHOOSING, CREATE_DECK, CHOOSE_DELETE, DELETE_DECK, SHOW_QNS, DELETE_QNS, INSERT_CARD, VIEW_QNA, ADD_QNS, ADD_ANS, ADD_MORE_QNS, DECK_TEST, TESTING, DONE = range(14)
...
def add_qns(update, context):
    if (getText(update) == "/cancel"):
        ...
        try:
            ...
            # Add card_id into add_more
            postgres_insert_query = """INSERT INTO add_more (card_id, user_id) VALUES (%s, %s)"""
            card_id_to_insert = (ID, getID(update),)
            cursor.execute(postgres_insert_query, card_id_to_insert)
            connection.commit()
            print("card_id inserted successfully into add_more table")
        except (Exception, psycopg2.Error) as error :
            if(connection):
    ...
    if (getText(update) == "/cancel"):
        ...
        try:
            ...
            #                     reply_markup=ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=True))
                update.message.reply_text("Would you like add more flashcards?\n\nType YES to continue and type NO to stop.")
            return ADD_MORE_QNS
...
def add_more_qns(update, context):
    if (getText(update) == "/cancel"):
        ...
        try: 
            connection = connect_db()
            cursor = connection.cursor()
            # Retrieve deck_id from card_id
            postgres_deck_id_query = """SELECT deck_id FROM cards INNER JOIN add_more ON cards.card_id = add_more.card_id"""
            cursor.execute(postgres_deck_id_query)
            deck_id = cursor.fetchone()
            print(deck_id)
            # Delete record 
            postgres_delete_query = """DELETE FROM add_more WHERE user_id = %s"""
...
def delete(update, context):
    if (getText(update) == "/cancel"):
        ...
        update.message.reply_text(
            ...
            'If you would like to delete a question from the deck, type QNS.\n\n'
            'Type /cancel to stop this action.')
        return CHOOSE_DELETE

def choose_delete(update, context):
    if getText(update).lower() == "deck":
        if deck_list(update, context) == 0:
            return CHOOSING
        else: 
...
def delete_deck(update, context):
    if (getText(update) == "/cancel"):
        ...
        try:
            ...
            # Delete deck
            postgres_delete_deck_query = """DELETE FROM decks WHERE deck_name = %s"""
